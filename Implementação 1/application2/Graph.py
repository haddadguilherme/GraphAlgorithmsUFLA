...
class Graph:
    ...
    def addEdge(self, u, v):
        ...
        if (isinstance(u, Vertex) and isinstance(v, Vertex)):
            ...
            # Adiciona +1 no contador de arestas incidentes nos vértices
            self.countEdges(u)
            self.countEdges(v)
        else:
    ...
    # Cria lista com o número de vértices passados
    def createCountEdges(self, numberVertex):
        self.num_edges = [0] * numberVertex

    # Contador de arestas incidentes em cada vértice
    def countEdges(self, vertex):
        self.num_edges[vertex-1] += 1

    # Verifica se existe algum vértice de grau ímpar
    def vertexOdd(self):
        for value in self.num_edges:
            if (value % 2) != 0:
